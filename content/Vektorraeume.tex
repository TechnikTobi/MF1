...
\begin{document}
	...
	\begin{definition}[Basis]
		...
		\begin{itemize}
			\item $\textrm{span}(B) = V$\sidenote{Wir sagen: \\ ''$B$ \textit{erzeugt} $V$''}
	...
	\begin{theorem}
		Sei $V$ ein \hyperref[def:Vektorraum]{Vektorraum} und $B \subseteq V$ ein \hyperref[def:Untervektorraum]{Untervektorraum}. Folgende Aussagen sind dann äquivalent:\sidenote{Natürlich folgt aus $B \subseteq V$ \textit{nicht} dass $B$ eine Basis von $V$ ist, es gilt lediglich, dass entweder alle oder keine der Aussagen wahr sind}
		\begin{itemize}
