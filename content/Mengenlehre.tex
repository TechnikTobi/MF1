...
\begin{document}
	...
	\begin{definition}[Mächtigkeit, Kardinalität, Kardinalzahl]
	...
	\begin{definition}[gleichmächtig]
		...
		Gilt $|M| = |N|$, so nennen wir die beiden Mengen $M$ und $N$ \textbf{gleichmächtig} (wir sagen auch, sie haben die gleiche \hyperref[def:Kardinalität]{Kardinalität}). Des Weiteren halten wir fest, dass zwei Mengen $M$ und $N$ genau dann gleichmächtig sind, wenn es eine \hyperref[def:Bijektiv]{bijektive} \hyperref[def:Abbildung]{Abbildung}\sidenote{Siehe Kapitel \ref{chap:Abbildungen}} $f: M \rightarrow N$ zwischen diesen Mengen gibt. 
	\end{definition}
